...
This server requires 3 headers (not 1) — X-API-Key, X-MSP-Host, and
X-MSP-Tenant-Id — because live testing proved the app-routing gateway in
front of the Forms API needs the tenant id forwarded downstream as an
X_Tenant_ID cookie (see api_client.py / server.py docstrings).
"""

from starlette.testclient import TestClient

from mspbots_forms_mcp.__main__ import _build_http_app
from mspbots_forms_mcp.config import Settings
from mspbots_forms_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_all_headers_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert body["required_headers"] == ["X-API-Key", "X-MSP-Host", "X-MSP-Tenant-Id"]
...
def test_all_headers_present_reaches_request_context(monkeypatch):
    ...
    from mspbots_forms_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
...
def test_client_factory_builds_client_with_cookie_tenant(monkeypatch):
    # Confirms the tenant id flows into FormsAPIClient in a way that ends up
    # as the X_Tenant_ID cookie (not just a header) — the hard-won gateway
    # requirement this server must never regress.
    from mspbots_forms_mcp.server import _gateway_creds_var

    settings = Settings()
    token = _gateway_creds_var.set(("tok", "https://agent.mspbots.ai", "tenant-xyz"))
    try:
        client = get_client_from_context(settings)
        assert client is not None
        assert client._cookies() == {"X_Tenant_ID": "tenant-xyz"}
    finally:
